...
public plugin_init() {
    ...
    register_think("sw_fakewall", "FakeWallThink");
}
...
public FakeWallThink(pEntity)
{
    ...
    if(pAdmin != 0)
    {
        ...
        DrawBox(fFakeWallOrigin, trEndPos, Float:{255.0, 0.0, 0.0}, 10); 
    ...
    else
    {
        ...
        DrawBox(fVecUser1, fVecUser2, Float:{0.0, 255.0, 0.0}, 50); 
...
public DrawBox(const Float:vecStart[3], const Float:vecEnd[3], const Float:color[3], iDispTime)
{
...
public drawBeam(const Float:from[3], const Float:to[3], life, const Float:color[3])
{
    message_begin(MSG_BROADCAST, SVC_TEMPENTITY);
    write_byte(TE_BEAMPOINTS);

    for(new i = 0; i < 3; i++) write_coord(floatround(from[i]));
    for(new i = 0; i < 3; i++) write_coord(floatround(to[i]));
...
public cmd_StartBlock(id)
{
    ...
    set_pev(box, pev_angles, Float:{0.0, 0.0, 0.0});
    set_pev(box, pev_velocity, Float:{0.0, 0.0, 0.0});
