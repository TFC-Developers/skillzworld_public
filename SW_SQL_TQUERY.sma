...
public plugin_natives()
{
	...
	register_native("api_SQLAddThreadedQuery", "Native_SQLAddThreadedQuery")
	register_native("api_SQLRemoveThreadedQuery", "Native_RemoveThreadedQuery")

	register_library("sql_tquery")
}
...
{
	DebugPrintLevel(1, "Native_AddThreadedQuery(%d, %d)", iPlugin, iParams)
	...
	if (g_bPluginEnd)
	{
		...
		WarningLog("Plugin \"%s\" attempted to add threaded query after plugin_end (\"%s\")", sPluginFileName, sQuery)
		return 0
	}
...
{
	DebugPrintLevel(1, "Native_RemoveThreadedQuery(%d, %d)", iPlugin, iParams)

	new iIdent = get_param(1)

	if (g_iCurrentQueryIdent == iIdent)
	{
		DebugPrintLevel(2, "Unable to remove query in progress")
		return 0
	}
	...
	for (new i = 0; i < g_iQueryCount; i++)
	{
		...
		{
			DebugPrintLevel(2, "Query ident %d found", iIdent)
			...
			else
			{
				...
				WarningLog("Plugin: \"%s\" attempted to remove a query it doesn't own", sFile)
				return 0
			}
		}
	}
	DebugPrintLevel(2, "Query ident %d not found", iIdent)
...
RunNextQueryByPriority()
{
	DebugPrintLevel(1, "RunNextQueryByPriority()")
...
public RunThreadedQuery(iIndex)
{
	...
	DebugPrintLevel(2, "Running Query: Index %d Ident: %d Priority: %d", iIndex, Data[m_iQueryIdent], Data[m_iQueryPriority])
	DebugPrintLevel(3, sQuery)
...
RemoveQueryByIndex(iIndex)
{
	DebugPrintLevel(1, "RemoveQueryByIndex(%d)", iIndex)
...
public plugin_end()
{
	...
	for (new i = 0; i < g_iQueryCount; i++)
	{
		...
		{
			DebugPrintLevel(2, "Skipping Query: Index %d Ident: %d", i, Buffer[m_iQueryIdent])
		}
	}
