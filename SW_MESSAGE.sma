...
#define MESSAGE_FREQUENCY 120.0 // secs

new Array:g_MessageList
new const g_sMessageFile[] = "sw_message.txt"

new g_iEnt // Global entity used as a timer
new g_iCurMessage  = -1 // Current message position

new const g_sMessageForward[] = "fwd_ScreenMessage"
...
public plugin_init()
{
	...
	g_pCvarMessageHudChannel = register_cvar("sw_message_hud_channel", "4")
...
ReadMessageFile(sMessageFile[])
{
	...
	if (!iFileHandle)
	{
		WarningLog(FM_FOPEN_WARNING, sMessageFile)	
	}
